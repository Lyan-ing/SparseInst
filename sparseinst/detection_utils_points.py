...
def annotations_to_instances(annos, image_size, sample_points=0, mask_format="polygon"):
    """
    ...
    """
    target = base_annotations_to_instances(annos, image_size, mask_format=mask_format)

    # assert ("point_coords" in annos[0]) == ("point_labels" in annos[0])

    if len(annos) and "point" in annos[0]:
        point_coords = []
        # point_labels = []
        for i, _ in enumerate(annos):
            # Already in the image coordinate system
            point_coords_wrt_image = np.array(annos[i]["point"])
            ...
            point_coords.append(point_coords_wrt_image[0])
...
def transform_instance_annotations(
    ...
    if "point" in annotation:
        ...
        if inside:
            annotation["point"] = point_coords
        else:
            annotation["point"] = point_coords[:] * 0
    return annotation
