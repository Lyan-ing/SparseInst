...
def compute_project_term(mask_scores, gt_bitmasks, flag=True):
...
def dice_coefficient(x, target, flag = True):
    eps = 1e-5
    n_inst = x.size(0)
    ...
    if flag:
        ...
        intersection = torch.matmul(x, target.t())
        union = (x ** 2.0).sum(dim=1)[:, None] + (target ** 2.0).sum(dim=1)
...
def compute_pairwise_term(mask_logits, pairwise_size, pairwise_dilation):
    ...
    from sparseinst.sparseinst import unfold_wo_center
...
def compute_ious(pred, target, shape):
    pred = pred.sigmoid()
    pred_left = pred[:, 0] * shape[1]
    pred_top = pred[:, 1] * shape[0]
    pred_right = pred[:, 2] * shape[1]
    pred_bottom = pred[:, 3] * shape[0]
    ...
    assert 0 <= target_left.all() <= 1
    assert 0 <= target_right.all() <= 1
    assert 0 <= target_top.all() <= 1
    assert 0 <= target_bottom.all() <= 1

    target_aera = (target_right - target_left +1) * (target_bottom - target_top+1)  # 中心距两侧距离
    pred_aera = (pred_right - pred_left+1).clamp(min =0) * (pred_bottom - pred_top+1).clamp(min =0)

    w_intersect = torch.min(pred_right, target_right) - torch.max(pred_left, target_left)+1
    h_intersect = torch.min(pred_bottom, target_bottom) - torch.max(pred_top, target_top)+1

    g_w_intersect = torch.max(pred_right, target_right) - torch.min(pred_left, target_left)+1
    g_h_intersect = torch.max(pred_bottom, target_bottom) - torch.min(pred_top, target_top)+1

    ac_uion = g_w_intersect * g_h_intersect
    area_intersect = w_intersect.clamp(min =0) * h_intersect.clamp(min =0)
    area_union = target_aera + pred_aera - area_intersect

    ious = (area_intersect + 1.0) / (area_union.clamp(min =1.0))
...
class SparseInstCriterion(nn.Module):
    ...
    def __init__(self, cfg, matcher):
        ...
        # self.sim_with_mask = cfg.MODEL.SPARSE_INST.LOSS.SIM_WITH_MASK
        # self.sim_hyper = cfg.MODEL.SPARSE_INST.LOSS.SIM_HPYPER
        # --config - file.. / configs / sparse_inst_r50_giam.yaml
        # PYTHONUNBUFFERED=1;CUDA_VISIBLE_DEVICES=5;DETECTRON2_DATASETS=/media/data/yl/Dataset

    def get_weight_dict(self, cfg):
        losses = ("loss_ce", "loss_objectness", "loss_prj", "loss_pairwise" )
        weight_dict = {}
        ...
        prj_weight = cfg.MODEL.SPARSE_INST.LOSS.SIM_HPYPER[0]
        pairwise_weight = cfg.MODEL.SPARSE_INST.LOSS.SIM_HPYPER[1]
        # box_reg =1.0

        weight_dict = dict(
            zip(losses, (ce_weight, objectness_weight, prj_weight, pairwise_weight )))
        return weight_dict
    ...
    def loss_masks_with_iou_objectness(self, outputs, targets, indices, num_instances, input_shape):
        ...
        # target_reg = torch.cat([t["boxes"].tensor for t in targets], 0)
        # pred_red = outputs["pred_reg"]
        # pred_reg = pred_red[src_idx]
        # loss_reg = compute_ious(pred_reg, target_reg, input_shape)
        ...
        image_color_similarity = torch.cat([x["color_sim"] for x in targets])[mix_tgt_idx]
        # image_color_similarity = image_color_similarity[gt_inds].to(dtype=mask_feats.dtype)

        loss_prj_term = compute_project_term(src_masks.sigmoid()[:, None], target_masks[:, None])

        pairwise_losses = compute_pairwise_term(
            src_masks[:, None], 3, 2)
        weights = (image_color_similarity >= self.pairwise_color_thresh).float() * (target_masks.float()[:, None])
...
class SparseInstMatcher(nn.Module):
    ...
    def forward(self, outputs, targets, input_shape):
        with torch.no_grad():
            ...
            tgt_ids = torch.cat([v["labels"] for v in targets])  # label
            ...
            pred = pred_masks.view(B * N, 1, H, W)
            mask_score = compute_project_term(pred.sigmoid(), tgt_masks[:, None], flag=False)
